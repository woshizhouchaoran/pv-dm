...
class PVDM(Word2VecExtended):
	...
	def __init__(self, vocabulary = None, random_window = False, scale_updates = False, self_predict = 0, batchsize = 100, symmetric_window = True, oov_word = True, min_count = 5, paragraph_size = 400, concatenate = True, sentences=None, size=400, alpha=0.035, window=5, seed=1, workers=1, min_alpha=0.0001, decay = True, vocab_report_frequency = 10000):
		...
		if size % 4 != 0:
			gensim.models.word2vec.logger.warning("consider setting layer size to a multiple of 4 for greater performance")
	...
	def init_sims(self, replace=False):
		...
		if getattr(self, 'synparagraphnorm', None) is None or replace:
			gensim.models.word2vec.logger.info("precomputing L2-norms of word weight vectors")
			if replace:
	...
	def train(self, sentences, total_words=None, word_count=0, paragraphs_only = False, vocab = None, paragraphs = None):
			...
			while True:
				...
				with lock:
					...
					if elapsed >= next_report[0]:
						gensim.models.word2vec.logger.debug("PROGRESS: at %.2f%% words, alpha %.05f, %.0f words/s," %
		...
		for job_no, job in enumerate(utils.grouper(no_oov, self.batchsize)):
			gensim.models.word2vec.logger.debug("putting job #%i in the queue, qsize=%i" % (job_no, jobs.qsize()))
			jobs.put(job)
		gensim.models.word2vec.logger.info("reached the end of input; waiting to finish %i outstanding jobs" % jobs.qsize())
		...
		elapsed = time.time() - start
		gensim.models.word2vec.logger.info("training on %i sentences took %.1fs, %.0f sentences/s, %.6f" %
	...
	def build_paragraph_vocab(self, sentences):
		...
		gensim.models.word2vec.logger.info("collected %i sentence types from a corpus of %i sentences." %
	...
	def build_vocab(self, sentences, oov_word = False, report_frequency = 10000):
		...
		if path != None and file_exists(path):
			gensim.models.word2vec.logger.info("loading from saved vocab list at \"%s\"" % (path))
			...
			gensim.models.word2vec.logger.info("collecting all words and their counts")
			...
			for sentence_no, sentence in enumerate(sentences):
				...
				if sentence_no % report_frequency == 0:
					gensim.models.word2vec.logger.info("PROGRESS: at sentence #%i, processed %i words and %i word types" %
													   (sentence_no, total_words, len(vocab)))
				for word in sentence:
					if word in vocab:
						get_from_vocab(word).count += 1
					else:
						assign_to_vocab(word, gensim.models.word2vec.Vocab(count=1))
				total_words += len(sentence)
				prev_sentence_no = sentence_no
			gensim.models.word2vec.logger.info("collected %i word types from a corpus of %i words and %i sentences" %
			...
			gensim.models.word2vec.logger.info("total %i word types after removing those with count<%s" % (len(self.vocab), self.min_count))
			...
			self.reset_weights()
			if path != None:
				gensim.models.word2vec.logger.info("saving vocab list in \"%s\"" % (path))
